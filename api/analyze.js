...
【重要】
- 分析結果は中学生でも理解できる平易な日本語で記述してください
- 専門用語は使わず、日常的な言葉で説明してください
- 具体的で分かりやすい表現を心がけてください
...

{
  ...
  "redFlags": [
    "危険な点を平易な言葉で具体的に説明（例：「絶対に儲かる」など現実的でない約束をしている）",
    "危険な点2"
  ],
  "warnings": [
    "気をつけるべき点を分かりやすく説明（例：開発者の名前や経歴が書かれていない）",
    "気をつけるべき点2"
  ],
  "positivePoints": [
    "良い点を簡単な言葉で説明（例：技術の説明が具体的で分かりやすい）",
    "良い点2"
  ],
  "summary": "どんな暗号資産プロジェクトか、信頼できるかどうかを平易な言葉で要約（200文字程度）",
  "recommendations": [
    "次にすべきことを具体的に提案（例：このプロジェクトには投資しないでください）",
    "推奨アクション2"
  ]
}

【評価基準】
- 「絶対儲かる」「100%成功」など非現実的な約束をしていないか
- 技術の説明が曖昧で実現できそうにない内容ではないか
- 開発チームの名前や経歴が明記されているか
- トークン（コイン）の配分が開発者に偏りすぎていないか
- 今後の計画が具体的に書かれているか
- 法律やルールを守る姿勢が示されているか
- 実際に使われている実績やパートナー企業があるか
- 文書の質が高く、誤字脱字や不自然な表現がないか

【表現の注意点】
❌ 避けるべき専門用語の例：
- 「トークノミクス」→「トークン（コイン）の配分」
- 「コンプライアンス」→「法律やルールを守る姿勢」
- 「ロードマップ」→「今後の計画」
- 「ブロックチェーン技術」→「暗号技術」
